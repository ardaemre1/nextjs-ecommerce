...
import {useAuthStore, useBasketStore} from "@/app/store.js";
import Link from "next/link.js";
import {toast, ToastContainer} from "react-toastify";
import confetti from 'canvas-confetti';
import Confetti from 'react-confetti';
import Counter from "@/app/component/Counter.jsx";
const basket = () => {

    const basketItems = useBasketStore((state) => state.basket)
        const removeAllItems = useBasketStore((state) => state.clearBasket)
        const clearItem = useBasketStore((state) => state.clearItem)
    ...
    const handleCheckout = () => {
        if (isAuthenticated) {
            if (basketItems.length > 0) {
                ...
                toast.success('🦄 Ödeme İşlemi Başarılı Bir Şekilde Gerçekleştirilmiştir...', {
                    position: "top-right",
                    autoClose: 3000,
                ...
                console.log("Sepet boş, sipariş tamamlanamaz.");
            }
        } else {
            toast.error('Siparişi Tamamlayabilmeniz İçin Giriş Yapmanız Gerekmektedir...', {
                position: "top-right",
                autoClose: 5000,
                hideProgressBar: false,
                closeOnClick: true,
    ...
    return(
            <div className='flex flex-col justify-center items-center h-100 w-100'>
                <h1 className='text-3xl font-bold mt-3'>Shopping Cart</h1>
                <ul className='mt-5 mb-5 flex flex-col justify-center items-center'>
                    {basketItems.length > 0 ? (
                            <div className="mb-4">
                                <table className="hidden md:block w-full text-sm text-left text-black text-center">
                                    <colgroup>
                                    ...
                                    {basketItems.map((item) => (
                                        <tr key={item.id} className="bg-white border-b hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 font-semibold text-black" style={{ whiteSpace: 'nowrap' }}>
                                                {item.product_name}
                                            </td>
                                            ...
                                            <td className="px-6 py-4 font-semibold text-black">
                                                <p className='text-red-600 font-bold'>Price: {parseFloat(item.price).toFixed(2).replace(/\.?0+$/, "")} ₺</p>
                                                </td>
                                            <td className="px-6 py-4">
                                                <button onClick={() => clearItem(item)} type="button" className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Remove</button>
                                            </td>
                                        </tr>
                                    ))}
                                    </tbody>
                                </table>
                                <div className='md:hidden flex flex-col items-center flex-wrap justify-center w-100 h-100'>
                                    {basketItems.map((item) => (
                                        <div key={item.id} id={item.id} className='border-2 border-black rounded-2xl grid-cols-3 grid place-items-center w-100 h-100 gap-10 p-5 m-5'>
                                            <div className='grid-cols-1'>
                                                <img src={item.images} alt={item.product_name} className='w-20 h-20'/>
                                            </div>
                                            <div className='gap-y-2 grid-cols-1 flex flex-col items-center justify-around'>
                ...
                {basketItems.length > 0 && (
                    <div className='flex justify-center items-center flex-col flex-wrap'>
                        <p>Toplam Fiyat: <span className='font-bold'>{totalPrice} ₺</span></p>
                        <button type="button" onClick={removeAllItems} className="mt-5 text-red-700 hover:text-white border border-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900">Remove All İtems</button>
                        <div className='flex flex-wrap flex-col justify-center mt-10'>
                            <button type="button" onClick={handleCheckout} className="text-gray-900 bg-[#F7BE38] hover:bg-[#F7BE38]/90 focus:ring-4 focus:outline-none focus:ring-[#F7BE38]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#F7BE38]/50 mr-2 mb-2">
                                {paypalicon}
                                Ödemeni PayPal İle Yap
                            </button>
                            <button onClick={handleCheckout} type="button" className="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-800 dark:bg-white dark:border-gray-700 dark:text-gray-900 dark:hover:bg-gray-200 mr-2 mb-2">
                                {visaicon}
                                Ödemeni Visa İle Yap
                            </button>
                            <button  onClick={handleCheckout}  type="button" className="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 mr-2 mb-2">
                                {mastercardicon}
                                Ödemeni MasterCard İle Yap
                            </button>
                            <button onClick={handleCheckout}  type="button" className="text-white bg-[#2557D6] hover:bg-[#2557D6]/90 focus:ring-4 focus:ring-[#2557D6]/50 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#2557D6]/50 mr-2 mb-2">
                                {aexpress}
                                Ödemeni American Express ile Yap
                            </button>
                            <button  onClick={handleCheckout} type="button" className="mb-10 flex justify-evenly items-center text-white bg-[#050708] hover:bg-[#050708]/80 focus:ring-4 focus:outline-none focus:ring-[#050708]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center  items-center dark:hover:bg-[#050708]/40 dark:focus:ring-gray-600 mr-2 mb-2">
                                Ödemeni Apple Pay ile Yap
                                {applepay}
                            </button>
